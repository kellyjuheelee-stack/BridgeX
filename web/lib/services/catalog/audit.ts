// web/lib/services/catalog/audit.ts
// 카탈로그 컴플라이언스 예비 점검 규칙 (모듈 2a)
// 레거시 backend/src/constants/catalogAuditRules.js 의 TS 포팅 — 동작 동일성 유지.
//
...
export interface BuyerReadyCheck {
  item: string;
  signals: string[];
  note: string;
}

export interface BuyerReadyResult {
  item: string;
  present: boolean;
  note: string;
}
...
export interface AuditResult {
  findings: Finding[];
  buyerReady: BuyerReadyResult[];
  summary: { flaggedCount: number; checkedChars: number };
  disclaimer: string;
}
...
export const CLAIM_RULES: ClaimRule[] = [
  {
    ...
    category: "의약품·치료 뉘앙스",
    terms: [
      "치료", "치유", "여드름 치료", "염증", "항염", "재생", "콜라겐 생성", "상처", "흉터",
  ...
  {
    key: "freefrom",
    category: "free-from·무첨가",
    terms: [
      "무독성", "무첨가", "파라벤프리", "파라벤 프리", "paraben-free", "paraben free",
  ...
  },
  {
    ...
    category: "근거 필요·과장",
    terms: [
      "즉각", "즉시", "100%", "영구", "완벽", "instant", "instantly", "permanent", "guaranteed",
  ...
  {
    key: "compliance_benefit",
    category: "규제 준수를 강점처럼",
...
export const BUYER_READY_CHECKS: BuyerReadyCheck[] = [
  { item: "INCI 전성분", signals: ["inci", "전성분"], note: "INCI 전성분 표기가 바이어 검토에 필요할 수 있습니다." },
  {
    item: "인증",
    signals: ["gmp", "iso 22716", "vegan", "비건", "cruelty", "인증"],
    note: "보유 인증을 명시하면 바이어 신뢰에 도움이 됩니다.",
  },
  {
    item: "거래조건(MOQ·Incoterms)",
    signals: ["moq", "incoterms", "fob", "cif", "exw"],
    note: "MOQ·Incoterms 등 거래조건을 넣으면 바이어 문의를 줄일 수 있습니다.",
  },
];
...
export function auditText(text: unknown): AuditResult {
  ...
  const buyerReady: BuyerReadyResult[] = BUYER_READY_CHECKS.map((c) => ({
    item: c.item,
