// web/lib/services/roadmap/buildRoadmap.ts
...
//
// backend/src/services/roadmap.service.js 를 순수 TypeScript 로 포팅. 동작 동일성 유지.
import type {
  RoadmapDiagnosis,
  RoadmapProgress,
  RoadmapResult,
...
function hasAny(arr: unknown, list: string[]): boolean {
  return Array.isArray(arr) && arr.some((x) => list.indexOf(x) !== -1);
}
...
interface DerivedContext {
  ...
  hasRealCerts: boolean;
  hasBrochure: boolean;
}

function deriveContext(d: RoadmapDiagnosis | null | undefined): DerivedContext {
  d = d || {};
  ...
  return {
    ...
    sellingInKorea: ["판매 중", "테스트 판매 중"].indexOf(d.isSellingInKorea as string) !== -1,
    hasRealCerts:
      Array.isArray(certs) && certs.some((c) => ["없음", "잘 모르겠음"].indexOf(c) === -1),
...
interface StepDef {
  ...
  tag: StepTag;
  link: StepLink;
  done: (c: DerivedContext) => boolean;
  comment: string;
  showIf?: (c: DerivedContext) => boolean;
}
...
const STEP_DEFS: StepDef[] = [
  ...
  { id: "interest_reply", phase: "buyer", title: "관심 답장 대응", tag: "self",
    link: { type: "tool", href: "reply.html" }, done: () => false,
    comment: "바이어 답장을 붙여넣으면 무엇을 원하는지 분석해 영문 대응 초안을 만들어 드립니다." },
...
export function buildRoadmap(
  diagnosis: RoadmapDiagnosis | null | undefined,
  ...
  const expertRemaining = steps.filter(
