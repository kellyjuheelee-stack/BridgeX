...
function draftResponse(analysis, ctx) {
  ctx = ctx || {};
  ...
  if (analysis.isRejection) {
    ...
    lines.push(`I completely understand. If the timing changes or your assortment needs evolve, I'd be glad to reconnect — I'll keep you posted on new launches and stay in touch.`);
  } else {
    lines.push(`Thank you so much for your reply and your interest in ${product}.`);
    const paras = [];
    if (has('pricing')) paras.push(`I'd be glad to share our price list, MOQ, and Incoterms — I'll prepare a tailored offer sheet for your market.`);
    if (has('samples')) paras.push(`We can arrange samples. Could you confirm the best shipping address and which items you'd like to try?`);
    if (has('materials')) paras.push(`I'm attaching (or can send) our full catalog and product line sheet.`);
    if (has('compliance')) paras.push(`On compliance: ${certs.length ? 'our products are ' + certs.slice(0, 3).join(', ') + ' certified, and ' : ''}we can walk you through the INCI list and EU documentation (CPNP) status.`);
    if (has('timeline')) paras.push(`Regarding lead time and delivery, I'll confirm current availability and shipping timelines for your order size.`);
    if (has('meeting')) paras.push(`A short call sounds great — would any time next week work for you? I can send a few options in your time zone.`);
    if (!paras.length) paras.push(`I'd love to share more details — our catalog, pricing, and how we can support your market entry.`);
...
module.exports = { INTENT_RULES, analyzeReply, draftResponse, tipsFor };
